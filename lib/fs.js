...
function mkdirs(path, callback) {
  ...
  }).then(() => fsPromises.mkdir(path)).catch(err => {
...
function _readAndFilterDir(path, options) {
  return Promise.resolve(fsPromises.readdir(path))
    ...
    .map(item => {
      ...
      return fsPromises.stat(fullPath).then(stats => ({
...
function readFile(path, options = {}, callback) {
  ...
  return Promise.resolve(fs.promises.readFile(path, options)).then(content => {
...
function _emptyDir(path, options, parent) {
  return _readAndFilterDir(path, options)
    ...
    .map(item => {
      ...
      if (item.isDirectory) {
        return _emptyDir(fullPath, options, currentPath).tap(() => fs.promises.readdir(fullPath).then(files => {
          if (!files.length) return fsPromises.rmdir(fullPath);
        }));
      }
...
function _rmdir(path) {
  return Promise.resolve(fsPromises.readdir(path)).map(item => {
    const childPath = join(path, item);

    return fsPromises.stat(childPath).then(stats => {
      if (stats.isDirectory()) {
        return _rmdir(childPath);
      }

      return fsPromises.unlink(childPath);
    });
...
function ensurePath(path, callback) {
  ...
  return exists(path).then(exist => {
    ...
    return fsPromises.readdir(dirname(path)).then(files => _findUnusedPath(path, files));
  }).asCallback(callback);
}
