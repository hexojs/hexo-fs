...
function _readAndFilterDir(path, options) {
  ...
  return Promise.resolve(fsPromises.readdir(path, Object.assign(options, { withFileTypes: true })))
...
function _rmdir(path) {
  return Promise.resolve(fsPromises.readdir(path, { withFileTypes: true })).map(item => {
    const childPath = join(path, item.name);
